                        ...
                        <tbody>
                            ...
                            if(($_GET)){
                                ...
                                if(isset($_GET["id"])){
                                    ...
                                    // Calculate patient's age based on date of birth (pdob)
                                    $dob_query = $database->query("SELECT pdob FROM patient WHERE pid = $userid");
                                    $dob_data = $dob_query->fetch_assoc();
                                    $dob = $dob_data["pdob"];
                                    $birth_date = new DateTime($dob);
                                    $today_date = new DateTime();
                                    $age = $today_date->diff($birth_date)->y;

                                    // Retrieve county ID from the counties table
                                    $county_id_query = $database->query("SELECT id FROM counties WHERE name = (SELECT paddress FROM patient WHERE pid = $userid)");
                                    $county_id_data = $county_id_query->fetch_assoc();
                                    $county_id = $county_id_data["id"];

                                    // Retrieve doctor's specialty ID from the specialties table
                                    $specialty_query = $database->query("SELECT specialties FROM doctor WHERE docid = (SELECT docid FROM schedule WHERE scheduleid = $scheduleid)");
                                    $specialty_data = $specialty_query->fetch_assoc();
                                    $doctor_specialty_id = $specialty_data["specialties"];

                                    // Call Python script to predict price
                                    $command = escapeshellcmd("python appoinmentPricePrediction.py $age $county_id $doctor_specialty_id");
                                    $out_result = shell_exec($command);
                                            ...
                                            <div  class="dashboard-items search-items"  >
                                                ...
                                                <div style="width:100%">
                                                        ...
                                                        <div class="h3-search" style="font-size:18px;">
                                                            ...
                                                            Channeling fee : <b>Ksh '.$out_result.'</b>
